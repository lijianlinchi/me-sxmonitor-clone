...
const ctn = document.getElementById("galaxyHero");
if (!ctn) {} else {
...
void main(){ vec2 focalPx=uFocal*uResolution.xy; vec2 uv=(vUv*uResolution.xy-focalPx)/uResolution.y; vec2 mouseNorm=uMouse-vec2(0.5); if(uAutoCenterRepulsion>0.0){ vec2 centerUV=vec2(0.0); float centerDist=length(uv-centerUV); vec2 repulsion=normalize(uv-centerUV)*(uAutoCenterRepulsion/(centerDist+0.1)); uv+=repulsion*0.05; } else if(uMouseRepulsion){ vec2 mousePosUV=(uMouse*uResolution.xy-focalPx)/uResolution.y; float mouseDist=length(uv-mousePosUV); vec2 repulsion=normalize(uv-mousePosUV)*(uRepulsionStrength/(mouseDist+0.1)); uv+=repulsion*0.05*uMouseActiveFactor; } else { vec2 mouseOffset=mouseNorm*0.1*uMouseActiveFactor; uv+=mouseOffset; } float autoRotAngle=uTime*uRotationSpeed; mat2 autoRot=mat2(cos(autoRotAngle),-sin(autoRotAngle),sin(autoRotAngle),cos(autoRotAngle)); uv=autoRot*uv; uv=mat2(uRotation.x,-uRotation.y,uRotation.y,uRotation.x)*uv; vec3 col=vec3(0.0); for(float i=0.0;i<1.0;i+=1.0/NUM_LAYER){ float depth=fract(i+uStarSpeed*uSpeed); float scale=mix(20.0*uDensity,0.5*uDensity,depth); float fade=depth*smoothstep(1.0,0.9,depth); col+=StarLayer(uv*scale+i*453.32)*fade; } if(uTransparent){ float alpha=length(col); alpha=smoothstep(0.0,0.3,alpha); alpha=min(alpha,1.0); fragColor=vec4(col,alpha); } else { fragColor=vec4(col,1.0); } }`;
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uFocal: { value: new Float32Array([0.32, 0.5]) },
      uRotation: { value: new Float32Array([1.0, 0.0]) },
      uStarSpeed: { value: 0.5 },
      uDensity: { value: 0.8 },
      uHueShift: { value: 30 },
      uSpeed: { value: 1.0 },
      uMouse: { value: new Float32Array([0.32, 0.5]) },
  ...
  let tm = { x: 0.5, y: 0.5 };
  let sma = 0.0, tma = 0.0;
  function resize(){ renderer.setSize(ctn.clientWidth||1, ctn.clientHeight||1); program.uniforms.uResolution.value = new Color(gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height); }
  window.addEventListener("resize", resize);
  resize();
  function update(t){ requestAnimationFrame(update); program.uniforms.uTime.value = t*0.001; program.uniforms.uStarSpeed.value = (t*0.001*0.5)/10.0; sm.x += (tm.x - sm.x)*0.05; sm.y += (tm.y - sm.y)*0.05; sma += (tma - sma)*0.05; program.uniforms.uMouse.value[0]=sm.x; program.uniforms.uMouse.value[1]=sm.y; program.uniforms.uMouseActiveFactor.value=sma; renderer.render({ scene: mesh }); }
  requestAnimationFrame(update);
  ctn.addEventListener("mousemove", function(e){ var r=ctn.getBoundingClientRect(); tm.x=(e.clientX-r.left)/r.width; tm.y=1.0-(e.clientY-r.top)/r.height; tma=1.0; });
  ctn.addEventListener("mouseleave", function(){ tma=0.0; });
}
